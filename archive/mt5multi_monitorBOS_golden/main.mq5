...
int quooffset   = 0          ;
bool timerfg    = true       ;
matrix M_sbd    = matrix::Zeros(PERIODX4, 9)  ;// {p bosd H L O T T T T }
matrix M_sbu    = matrix::Zeros(PERIODX4, 9)  ;// {p bosu H L O T T T T }
matrix M_sbdo   = matrix::Zeros(PERIODX4, 16) ;// {p bosd H L O T T T T i2bd H L O T sbd_ted sbu_ted}
matrix M_sbuo   = matrix::Zeros(PERIODX4, 16) ;// {p bosu H L O T T T T i2bu H L O T sbd_ted sbu_ted}
matrix M_sbdCP  = matrix::Zeros(PERIODX4, 7)  ;// {p bos H L BT sbd_ted sbu_ted}
matrix M_sbuCP  = matrix::Zeros(PERIODX4, 7)  ;// {p bos H L BT sbd_ted sbu_ted}
...
void OnTick() {
    if(timerfg){
        ...
        for(int k=0; k<10; ++k){
            ...
            MatBosinit(M_sbdo, PERIODX4, 16)  ;
            MatBosinit(M_sbuo, PERIODX4, 16)  ;
            MatBosinit(M_sbdCP, PERIODX4, 7) ;
            MatBosinit(M_sbuCP, PERIODX4, 7) ;
        }
        timerfg = false ;
    }
}
