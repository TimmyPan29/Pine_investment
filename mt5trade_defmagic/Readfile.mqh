...
void RSfile(const string& symbolname, const string& sectorname, Golder& G){
	ResetLastError();
    ...
    ResetLastError();
	string filename2 = StringFormat("%s"+"\\%s"+"\\BosSort"+"\\TradeBOS_%s.txt", AccountInfoString(ACCOUNT_COMPANY), sectorname, symbolname);
    filehandle=FileOpen(filename2, FILE_READ|FILE_TXT);
	if(filehandle!=INVALID_HANDLE){
		//PrintFormat("File path: %s\\Files\\", TerminalInfoString(TERMINAL_DATA_PATH));
		string line ;
		int rows= 0;
