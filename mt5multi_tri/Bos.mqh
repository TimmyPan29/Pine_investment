...
struct FVG{
    ...
    double Findmaxcloseaftk(const RawCandles& rc, int idx, int kbarsize, int& kbarend){//find max in bull candle //idx+3 cuz +0~+2is box position
        int i = idx;
        double tempclose;
        double temp ;
    ...
    double Findmincloseaftk(const RawCandles& rc, int idx, int kbarsize, int& kbarend){//find min in bear candle //idx+3 cuz +0~+2is box position
        int i = idx;
        double tempclose;
        double temp ;
        if (i+3==kbarsize) temp = rc.rawprices[kbar[kbarsize-1]]; 
    ...
    void Putefffvg(const RawCandles& rc){
        ...
        for(int putidx=0; putidx<kbarsize-2; ++putidx){
            maxpt = Findmaxcloseaftk(rc, putidx, kbarsize, kbarmaxend);
            minpt = Findmincloseaftk(rc, putidx, kbarsize, kbarminend);
...
struct Triset{
    ...
    int     wide2itvXF      ;
    int     fvgtype0F       ;
    Triset():wide2itv_d(-1),wide2itv_u(-1),wide2itv0X(-1),wide2itvXF(-1),fvgtype0F(-1){}
    ...
    void   TriFVGcheck0F   (FVG& fvg, const double& u, const double& d);
};
...
double Triset::TriItvCompare0X (FVG& fvg, const BOS& bos1, const BOS& bos2, const BOS& bos3, const BOS& bos4, double& delta0X, const int& j, const int& count3){//7 belong case 0xXXX0XXXX, 11 belong 0xXXXXFXXX
    ...
    if(count3==7){
        ...
        TriFVGcheck0F(fvg, minsbu, delta0X);
...
double Triset::TriItvCompareXF (FVG& fvg, const BOS& bos1, const BOS& bos2, const BOS& bos3, const BOS& bos4, double& deltaXF, const int& j, const int& count3){//7 belong case 0xXXX0XXXX, 11 belong 0xXXXXFXXX
    ...
    if(count3==11){
        ...
        TriFVGcheck0F(fvg, deltaXF, maxsbd);
        if(wide2itvXF==-1){
            wide2itvXF = j ;
            deltatemp = deltaXF ;
        }
...
void Triset::TriFVGcheck0F(FVG& fvg, const double& u, const double& d){
    int effkbarsize = fvg.Getfvgeffkbarsize();
    for (int i=0; i<effkbarsize; ++i){
        if(fvg.Property[i]%2==0 || fvgtype0F==1){
            fvgtype0F = ((fvg.LTprice[i]<u && fvg.LTprice[i]>d) || (fvg.RBprice[i]<u && fvg.RBprice[i]>d))? 2 : 0 ;
            fvgtype0F = fvgtype0F==1? 0x0000000F : fvgtype0F ;
        }
        else{
            fvgtype0F = ((fvg.LTprice[i]<u && fvg.LTprice[i]>d) || (fvg.RBprice[i]<u && fvg.RBprice[i]>d))? 1 : 0 ;
            fvgtype0F = fvgtype0F==2? 0x0000000F : fvgtype0F ;
        }
    }
    //printf("fvgtype0F= %d", fvgtype0F);
}
