...
int OnInit() {
    ...
    for(int k=0; k<10; ++k){
        ...
        for (int i=0; i<(PERIOD); ++i){
            ...
            for(int j=0; j<=i; ++j){
                if(i==0 && j==0){
                    int m = Bosarr[0].htfint ;
                    Tri[0]=TriCode(fvgarr[m], Tri[0], Bosarr[0], Bosarr[1], Bosarr[2], Bosarr[3], j, tempd, tempu, temp0X, tempXF);
                }
                else if(i!=0 && j==0){
                    int m = Bosarr[i].htfint ;
                    Tri[i]=TriCode(fvgarr[m], Tri[i], Bosarr[i], Bosarr[i+1], Bosarr[i+2], Bosarr[i+3], j, tempd, tempu, temp0X, tempXF);
                }
                else{
                    int m = Bosarr[i].htfint ;
                    Tri[i]=TriCode(fvgarr[m], Tri[i], Bosarr[i], Bosarr[i+(j+1)], Bosarr[i+((j+1)<<1)], Bosarr[i+(j+1)*3], j, tempd, tempu, temp0X, tempXF);
                }
            }
            tempd = 0;
            tempu = 0;
