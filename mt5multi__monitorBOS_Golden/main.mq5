...
matrix M_sbdo   = matrix::Zeros(PERIODX4, 9) ;// {p bosd H L O T T T T}
matrix M_sbuo   = matrix::Zeros(PERIODX4, 9) ;// {p bosu H L O T T T T}
...
void OnTick() {
    if(timerfg){
        ...
        for(int k=0; k<10; ++k){
            ...
            MatBosinit(M_sbdo, PERIODX4, 9)  ;
            MatBosinit(M_sbuo, PERIODX4, 9)  ;
