...
method formattedtime(Helper helper, string HTF) =>
    helper.name := "THE DATE OF BAR"
    r = str.format("{0,date,yyyy-MM-dd HH:mm}", time(HTF))
    r
...
method Monitor(CandleSet candleSet) =>
    ...
    if candleSet.candles.size() == 0
        temp := close
    if isNewHTFCandle != 0 or barstate.isrealtime or barstate.isfirst
    ...
    if candleSet.candles.size() > 0
        ...
        if(bosdata.state == 2)
            if(bosdata.slope1 != bosdata.slope2)
                ...
                bosdata.reg1key_idx := index==0? 0 : index - 1
            //else //Buff_key1維持原樣
        ...
        if(bosdata.state == 3)
            if(bosdata.slope1 != bosdata.slope2)
                ...
                bosdata.s_dateu := helper.formattedtime(candleSet.settings.htf)
            if(bosdata.regclose3<bosdata.sbd)
                bosdata.sbd := na
                bosdata.sbd_idx:= na
            bosdata.state := 1
        ...
        if(bosdata.state == 4)
            if(bosdata.slope1 != bosdata.slope2)
                ...
                bosdata.reg1key := bosdata.reg2key
                bosdata.reg1key_idx := bosdata.reg2key_idx
                bosdata.s_dated := helper.formattedtime(candleSet.settings.htf)
            if(bosdata.regclose3>bosdata.sbu)
                bosdata.sbu := na
                bosdata.sbu_idx:= na
            bosdata.state := 1
