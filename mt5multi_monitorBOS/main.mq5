...
int quooffset   = 0          ;
bool timerfg    = true       ;
matrix M_sbd    = matrix::Zeros(PERIODX4, 8) ;// {p cnt bosd H L O T kbaridx}
matrix M_sbu    = matrix::Zeros(PERIODX4, 8) ;// {p cnt bosu H L O T kbaridx}
matrix M_sbdo   = matrix::Zeros(PERIODX4, 8) ;// {p cnt bosd H L O T kbaridx}
matrix M_sbuo   = matrix::Zeros(PERIODX4, 8) ;// {p cnt bosu H L O T kbaridx}
matrix M_sbdCP  = matrix::Zeros(PERIODX4, 3) ;// {p bos T}
matrix M_sbuCP  = matrix::Zeros(PERIODX4, 3) ;// {p bos T}
...
void OnTick() {
    if(timerfg){
        ...
        for(int k=0; k<10; ++k){
            ...
            for (int i=0; i<(PERIODX4); ++i){
                BosPut(fvgarr[i], M_sbd, M_sbu, M_sbdo, M_sbuo, i);
            }
            BOSInsertalg(M_sbd);
            BOSInsertalg(M_sbu);
            for (int i=0; i<(PERIODX4); ++i){
                int dp = M_sbd[i][0]-1 ;
                int up = M_sbu[i][0]-1 ;
                BosSortAndFill(fvgarr[i], fvgarr[dp], fvgarr[up], M_sbd, M_sbu, M_sbdo, M_sbuo, i) ;//dp for sbd period
            }
            //+----------CheckPeriod---------+//
            ...
            // }
            //+----------Init---------+//
            M_sbd.Init(PERIODX4, 8)   ;
            M_sbu.Init(PERIODX4, 8)   ;
            M_sbdo.Init(PERIODX4, 8)  ;
            M_sbuo.Init(PERIODX4, 8)  ;
            M_sbdCP.Init(PERIODX4, 3) ;
            M_sbuCP.Init(PERIODX4, 3) ;
        }
        timerfg = false ;
    }
}
